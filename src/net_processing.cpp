...
/// protection against fingerprinting. Set to 6 days, denominated in seconds.
static constexpr int STALE_RELAY_AGE_LIMIT = 6 * 24 * 60 * 60; // 30 * 24 * 60 * 60; // 
/// Age after which a block is considered historical for purposes of rate
/// limiting block relay. Set to one week, denominated in seconds.
static constexpr int HISTORICAL_BLOCK_AGE = 7 * 24 * 12 * 60; // 7 * 24 * 60 * 60;  // 
    ...
    const CBlockIndex *pindexLast = nullptr;
    {
        ...
        for (const CBlockHeader& header : headers) {
            if (!hashLastBlock.IsNull() && header.hashPrevBlock != hashLastBlock) {
                // { + 
                LogPrintf("Mismatched hash: %s <> %s\n", hashLastBlock.ToString(), header.hashPrevBlock.ToString());
                // } + 
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::VERACK)
    {
        ...
        if (!pfrom->fInbound) {
            ...
            LogPrint(BCLog::NET, "New outbound peer connected: version: %d, blocks=%d, peer=%d%s\n",
    ...
    {
        LogPrint(BCLog::NET, "transaction received from peer=%d\n", pfrom->GetId()); // + 
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/sthcoin/sthcoin/issues/8279 for details.
                            assert(recentRejects);
    ...
    else if (strCommand == NetMsgType::GETADDR)
    {
        ...
        LogPrint(BCLog::NET, "Adding addresses for sending to peer=%d\n", pfrom->GetId()); // 
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pto->nNextAddrSend < nNow) {
            ...
            LogPrint(BCLog::NET, "Sending %d addresses. peer=%d\n", vAddr.size(), pto->GetId());
            if (!vAddr.empty())
              connman->PushMessage(pto, msgMaker.Make(NetMsgType::ADDR, vAddr));
        ...
        /* 
          ...
          LogPrint(BCLog::NET, "Wait more before sending addresses. peer=%d\n", pto->GetId());
        }
        */
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (fSendTrickle) {
                LOCK(pto->cs_filter);
                if (!pto->fRelayTxes) {
                    LogPrint (BCLog::TX, "Peer requested for no relaying tx.\n"); // + 
            ...
            if (fSendTrickle && pto->fSendMempool) {
                ...
                for (const auto& txinfo : vtxinfo) {
                    ...
                    LogPrint (BCLog::TX, "Checkin TX %s\n", hash.ToString ()); // + 
                    CInv inv(MSG_TX, hash);
                    pto->setInventoryTxToSend.erase(hash);
                    if (filterrate) {
                        if (txinfo.feeRate.GetFeePerK() < filterrate) {
                            LogPrint (BCLog::TX, "TX filtered out. tx fee = %lld, filter rate = %lld\n", txinfo.feeRate.GetFeePerK(), filterrate); // + 
                            continue;
                        }
                    }
                    if (pto->pfilter) {
                        if (!pto->pfilter->IsRelevantAndUpdate(*txinfo.tx)) {
                            LogPrint (BCLog::TX, "Irrelevant tx.\n"); // +
                            continue;
                        }
                    }
                    pto->filterInventoryKnown.insert(hash);
                    vInv.push_back(inv);
                    LogPrint (BCLog::TX, "TX added to inventory\n"); // + 
                    if (vInv.size() == MAX_INV_SZ) {
                        LogPrint (BCLog::TX, "Inventory full. Send inventory\n"); // + 
                        connman->PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv)); // actual sending
                        vInv.clear();
                    }
                }
            ...
            if (fSendTrickle) {
                ...
                while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                    ...
                    vInvTx.pop_back();
                    uint256 hash = *it;
                    LogPrint (BCLog::TX, "Checkin TX to relay %s\n", hash.ToString ()); // + 
                    // Remove it from the to-be-sent set
                    pto->setInventoryTxToSend.erase(it);
                    // Check if not in the filter already
                    if (pto->filterInventoryKnown.contains(hash)) {
                        LogPrint (BCLog::TX, "Skip tx already in filter inv.\n"); // + 
                        continue;
                    }
                    // Not in the mempool anymore? don't bother sending it.
                    auto txinfo = mempool.info(hash);
                    if (!txinfo.tx) {
                        LogPrint (BCLog::TX, "tx not in mempool any more.\n"); // + 
                        continue;
                    }
                    if (filterrate && txinfo.feeRate.GetFeePerK() < filterrate) {
                        LogPrint (BCLog::TX, "tx fee less than filter rate. %lld < %lld\n", txinfo.feeRate.GetFeePerK(), filterrate); // + 
                        continue;
                    }
                    if (pto->pfilter && !pto->pfilter->IsRelevantAndUpdate(*txinfo.tx)) {
                        LogPrint (BCLog::TX, "tx IsRelevantAndUpdate() failed\n"); // + 
                        continue;
                    }
                    // Send
                    vInv.push_back(CInv(MSG_TX, hash));
                    LogPrint (BCLog::TX, "TX inserted into inv.\n"); // + 
                    ...
                    if (vInv.size() == MAX_INV_SZ) {
                        LogPrint (BCLog::TX, "Inventory full now. Send inventory\n"); // + 
                        connman->PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv)); // actual sending
                        vInv.clear();
                    }
                    pto->filterInventoryKnown.insert(hash);
                    LogPrint (BCLog::TX, "tx %s inserted into filter inv.\n", hash.ToString ()); // + 
                }
            }
        }
        if (!vInv.empty())
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            if (timeNow > pto->nextSendTimeFeeFilter) {
                ...
                LogPrint (BCLog::ESTIMATEFEE, "min relay tx fee = %lld, current filter rate = %lld\n", ::minRelayTxFee.GetFeePerK(), filterToSend); // + 
