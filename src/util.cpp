...
const char * const STHCOIN_CONF_FILENAME = "sthcoin.conf";
const char * const STHCOIN_PID_FILENAME = "sthcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "sthcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Sthcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Sthcoin
    // Mac: ~/Library/Application Support/Sthcoin
    // Unix: ~/.sthcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sthcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Sthcoin";
#else
    // Unix
    return pathRet / ".sthcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", STHCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", STHCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Sthcoin Core") == std::string::npos) {
