...
###   sthcoincore.org and sthcoin.org and compares them.
...
WORKINGDIR="/tmp/sthcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://sthcoincore.org"
HOST2="https://sthcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="sthcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sthcoin-',
...
if [ $? -ne 0 ]; then
   echo "sthcoin.org failed to provide signature file, but sthcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "sthcoin.org and sthcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Sthcoin Core binary release signing key installed?"
   fi
